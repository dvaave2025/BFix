...
(MachRegister)0x1801000d, (MachRegister)0x18010003, (MachRegister)0x1801000c, (MachRegister)0x1801000f};//rsp, rbp, r13, rbx, r12,
...
std::map<unsigned long, dp::Block *> path_pc;
std::map<size_t,std::vector<Node*>> fixdata;
bool notpair = false;
...
dp::Block *allocb;
std::multimap<dp::Block *, dp::Block *> alloc_use;
std::map<unsigned long,bool> dom;
std::map<std::pair<Dyninst::Address, Dyninst::Address>, bool> pushpoint;
...
unsigned long opc, int size, Dyninst::Address firstAddr, dp::Block *xxb, bool findp, unsigned long insize){
				...
				if(memw){
					for (size_t k = 0; k < vova.size(); ++k) {
						for(odata xx : vova[k]){
							...
							for(int i = 0; i < 4; i++){
								if(xx.regs[i] != 0){
									if(xx.regs[i] == oper[0].regs[0] && xx.uid > 0){
										if(allocb != NULL){
											alloc_use.insert({allocb, b}); 
                                
											if(f->dominates(allocb, b)){ 
												printf("block: %lx is the domain of block %lx\n ", allocb->start(), b->start());
												dom[pathnum] = true;
											}else{
												printf("block: %lx is not the domain of block %lx\n ", allocb->start(), b->start());
												dom[pathnum] = false;
											}    
											kkkk = 1; 
										}    
				...
				if(memr){
					...
					for (size_t k = 0; k < vova.size(); ++k) {
                        for(odata xx : vova[k]){
                            ...
                            for(int i = 0; i < 4; i++){
                                if(xx.regs[i] != 0){
                                    if(xx.regs[i] == oper[1].regs[0] && xx.uid > 0){
                                        if(allocb != NULL){
                                            alloc_use.insert({allocb, b}); 
	...
	if(check_alloc && firstAddr > falsepath && stamp == 1 && falsepath != 0 && 
		/*uid < 2*/ firstobj == 1 ){
		//eachlink[pathnum].first = 0;//consider the path have allocation failed is the safe path.
	}
	stamp++;
	/*for (size_t k = 0; k < vova.size(); ++k) {
							...
							for(odata xx : vova[k]){
								for(int i = 0; i < 4; i++){
									if(xx.regs[i] != 0 && xx.regs[i] == t1.regs[jk] && xx.uid > 0){
										//eachlink[pathnum].first = 0;//see if arg is passed to other function, check if callee is libc
									}	
								}
							}
						}
	...
	for (size_t k = 0; k < vova.size(); ++k) {
		cout << "-------------------------\n";
		for(odata xx : vova[k]){
			if((xx == oper[0]) || (xx == oper[1])){
				add_node_to_group(table->groups[cchash(pathnum,table->size)], firstAddr, &xx, b, standc);
				standc++;
			}else if(size == 1 && opc == 0x129){
				//add_node_to_group(table->groups[cchash(pathnum,table->size)], firstAddr, &xx, b, standc);
			}
			for(int i = 0; i < 4; i++){
				if(xx.regs[i] != 0)
	...
	size_t group_index = cchash(pathnum, table->size);
	...
	if (group != NULL) {
		for (int i = 0; i < group->size; i++) {
			...
			while (current != NULL) {
				cout << "++++++++++++++++++++++\n";
				printf("bb.firstaddr: %lx, pc: %lx\n", current->bb->start(), current->pc);
				for(int aa = 0; aa < 4; aa++){
					if(current->meta->regs[aa] != 0){
						printf("%lx ,%s\n", current->meta->regs[aa], current->meta->regs[aa].name().c_str());
					}
				}
				cout<< "*******"<<current->ccc<<"\n";
				printf("mem:%d, numsrc:%d, imm:%lx, disp:%lx, scale:%lx, uid:%lx\n", current->meta->mem, current->meta->numsrc, current->meta->imm, current->meta->disp, current->meta->scale, current->meta->uid);
				current = current->next;
				
			}
		}
		/*size_t ww = cchash(firstAddr, group->size);
		Node *current = group->nodes[ww];
		while (current != NULL) {
			cout << "++++++++++++++++++++++\n";
            printf("bb.firstaddr: %lx, pc: %lx\n", current->bb->start(), current->pc);
            for(int aa = 0; aa < 4; aa++){
...
void getloopchild(dp::LoopTreeNode *ls){
	...
	if(lc.size() > 0){
		for (dp::LoopTreeNode* ln : lc) {
			...
			if(ltemp.size() > 0){
				...
				ln->loop->getLoopEntries(ebs);
				int bnum = ln->loop->getBackEdges(be);
				for (dp::Block *eb : ebs) {
                    cout << "ebtry block "<<hex <<eb->start() << "\n";
					if (std::find(ubs.begin(), ubs.end(), eb) == ubs.end()) {
                        ubs.push_back(eb);
				...
				for(dp::Edge *bee : be){
					cout<<"back edge: "<<bee->src()->start() << "->" << bee->trg()->start()<<"\n";
					if(bnum == 1)
						back = bee;
				}
				cout << "^^^^^^^^^^^^^^^^^^^^\n";
	...
	size_t indexx = 0;
	for (auto it = path_pc.begin(); it != path_pc.end(); ++it) {
		std::cout << "Index: " << indexx << ", Key: " << it->second->start()<<std::endl;
		if(it->second->start()==back->src()->start()){
			indexx = it->first;
            /*for(auto *zz : fixdata[it->first]){
                if(muid <= zz->meta->uid){
					muid = zz->meta->uid;
                }
			...
			std::cout << "Index: " << hex<<it->first << "\n";
			cout<<"Key: " << it->second->start()<<"\n";
        }
    }
	/*for (dp::Block *ub : ubs) {
...
Dyninst::Address getpreins(dp::Function const *f, dp::Block *b, Dyninst::Address pc){
    Dyninst::InstructionAPI::Instruction ins;
    Dyninst::InstructionAPI::InstructionDecoder decoder(f->isrc()->getPtrToInstruction(f->addr()),
		Dyninst::InstructionAPI::InstructionDecoder::maxInstructionLength,f->region()->getArch());
    Dyninst::Address firstAddr = b->start();
    Dyninst::Address lastAddr = b->end();
	Dyninst::Address pinsaddr;
    while(firstAddr < lastAddr){
        ins = decoder.decode((unsigned char *)f->isrc()->getPtrToInstruction(firstAddr));
        pinsaddr = firstAddr;
		firstAddr += ins.size();
...
unsigned long smallest_rsp_offset = 0;


void stackanalysis(dp::Function const *f, dp::Block *b, Dyninst::InstructionAPI::Instruction ins,
Dyninst::InstructionAPI::InstructionDecoder decoder, bool isleaf, Dyninst::Address insaddr){
    Expression::Ptr check;
    std::vector<Operand> operands;
    odata data;
    Dyninst::InstructionAPI::Expression::Ptr express;
    ...
    bool isrsp = false;
	long tempoffset = 0;
	Dyninst::InstructionAPI::Instruction callins;
	for(int i = 0; i < 4; i++){
        if((data.regs[1] == 0x18010004 && memr)||((data.regs[0] == 0x18010004 && memw))){//rsp is used as base register for memeory access
            printf("%d, %lx ,%s\n", i, data.regs[i], data.regs[i].name().c_str());
			isrsp = true;
		}
    }    
    printf("memr:%d, memw:%d, numsrc:%d, imm:%lx, disp:%lx\n", memr, memw, data.numsrc, data.imm, data.disp);
	if(isrsp){
		tempoffset = data.disp;
		if(smallest_rsp_offset > tempoffset){
			smallest_rsp_offset = tempoffset;
		}
	}
	if(isleaf){
		
	}else{
		//callins = decoder.decode((unsigned char *)f->isrc()->getPtrToInstruction(b->lastInsnAddr()));
		for(dp::Edge* e : b->targets()) {
			if(!e)
				continue;
            if(e->type() == dp::CALL){//store the update and recover point for rsp
				if(memw){
					auto it = std::find_if(pushpoint.begin(), pushpoint.end(), [&](const auto& p) {
						return p.first.second == b->lastInsnAddr();
					});
					if(it != pushpoint.end()){
						//if(it->first > insaddr){
							//it->first = insaddr;
						//}
						if(data.regs[0] == 0x18010004 && tempoffset < 0 && it->second == false){//must update rsp before this point
							pushpoint.erase(it); 
							pushpoint[{insaddr, b->lastInsnAddr()}] = true;
						}
					}else{
						pushpoint.insert({{insaddr, b->lastInsnAddr()}, false});
					}
				}else{ //must update rsp after this point
					if((data.regs[1] == 0x18010004 && memr) || 
						((data.regs[1] == 0x18010004 || data.regs[1] == 0x18010004) && data.mem != 1)){
						auto it = std::find_if(pushpoint.begin(), pushpoint.end(), [&](const auto& p) { 
							return p.first.second == b->lastInsnAddr();
						});  
						if(it != pushpoint.end()){
							...
							if(data.regs[0] == 0x18010004 && tempoffset < 0 && it->second == false){//must update rsp before this point
								pushpoint.erase(it); 
								pushpoint[{insaddr, b->lastInsnAddr()}] = false;
							}	    
						}else{
							pushpoint.insert({{insaddr, b->lastInsnAddr()}, false});
						}
...
int main(int argc, char *argv[]) {
	...
	for (auto const *f : all) {
		uid = 1;
		smallest_rsp_offset = 0;
		struct timeval astime, aetime, fstime, fetime;	
		...
		std::vector<dp::Edge *> ues;
		bool allocdom = true;
		unsigned long condition_alloc_path = 0;
		for (const auto& isdom : dom){
			if(!isdom.second){
				allocdom = false;
				condition_alloc_path = isdom.first;
				break;
			}
		}
		...
		bool hitone = false;
		for(size_t kkk = 1; kkk<= pathnum; kkk++){
			 if (path_pc.find(kkk) != path_pc.end()) {
				printf("path:0x%lx. pc: 0x%lx\n", kkk, path_pc[kkk]->lastInsnAddr());
				size_t group_index = cchash(kkk, table->size);
				Group *group = table->groups[group_index];
				if (group != NULL) {
					for (size_t i = 0; i < group->size; i++) {
						Node *current = group->nodes[i];
						while (current != NULL) {
							if(current->meta->uid > 0){
								hitone = true;
								fixdata[kkk].push_back(current);
								cout << "++++++++++++++++++++++\n";
								printf("bb.firstaddr: %lx, pc: %lx\n", current->bb->start(), current->pc);
								for(int aa = 0; aa < 4; aa++){
									if(current->meta->regs[aa] != 0){
		...
		long aseconds = aetime.tv_sec - astime.tv_sec;
		long amicroseconds = aetime.tv_usec - astime.tv_usec;
		double aelapsed = aseconds + amicroseconds*1e-6;
		...
		for (dp::Edge *ee : tues) {
			...
			Node * tt1 = NULL;
			/*for (auto it = path_pc.begin(); it != path_pc.end(); ++it) {
				if(*(it.second)->start()==ee->src()->start()){
					for(auto *zz : fixdata[it->first]){
						if(muid <= zz->meta->uid){
							muid = zz->meta->uid;
						}
			...
			/*for (auto it = path_pc.begin(); it != path_pc.end(); ++it) {
				std::cout << "Index: " << indexx << ", Key: " << it->first << it->second->start()<<std::endl;
				if(it->second->start()==ee->src()->start()){
					indexx = it->first;
					for(auto *zz : fixdata[it->first]){
						if(muid <= zz->meta->uid){
			...
			for(; muid >= 1; muid--){
				...
				cout << tt1->bb->start();
				for(int i = 0; i < 4; i++){
					if(tt1->meta->regs[i] != 0)
						printf("%lx ,%s\n", tt1->meta->regs[i], tt1->meta->regs[i].name().c_str());
				}
				printf("mem:%d, numsrc:%d, imm:%lx, disp:%lx, scale:%lx, uid:%lx\n",
					tt1->meta->mem, tt1->meta->numsrc, tt1->meta->imm, tt1->meta->disp, tt1->meta->scale, tt1->meta->uid);
				if(muid != tt1->meta->uid){
					cout << "#@$#@$#@$@#$#@$@#$#@$@#\n";
				}
                if (!createAndInsertSnippet2(app, &exitPoint, tt1->meta)) {//before point
                    fprintf(stderr, "createAndInsertSnippet2 failed\n");
                    exit(1);
                }
			...
			/*gettimeofday(&fetime, NULL);
			long fseconds = fetime.tv_sec - fstime.tv_sec;
			long fmicroseconds = fetime.tv_usec - fstime.tv_usec;
			double felapsed = fseconds + fmicroseconds*1e-6;

			printf("fix time: %.6f seconds\n", felapsed);*/
			//double ftime = difftime(etime, stime);
			//printf("Time in analysis: %.2f seconds\n", ftime);
		...
		free_table(table);*/		
			...
			printf("for conditional allocation\n");
			cout << std::hex << "path-0x" << condition_alloc_path <<": ";
            for (int i = 0; i < eachlink[condition_alloc_path].second.size() - 1; ++i) {
                cout << std::hex <<eachlink[condition_alloc_path].second[i]->start()<<"-" << blabel[eachlink[condition_alloc_path].second[i]].first << " -> ";
            }
            cout << eachlink[condition_alloc_path].second.back()->start() <<"-" << blabel[eachlink[condition_alloc_path].second.back()].first <<endl;
			for (int i = 0; i < eachlink[condition_alloc_path].second.size(); ++i) {
				findallPaths(f, f->entry(), path, eachlink[condition_alloc_path].second[i], true);
				Dyninst::Address lastAddr = eachlink[condition_alloc_path].second[i]->end();
				Dyninst::Address firstAddr = eachlink[condition_alloc_path].second[i]->start();
				cout <<eachlink[condition_alloc_path].second[i]->start()<<" path-" << infomap[firstAddr].first << " lastuse pc:" << eachpath[infomap[firstAddr].first].second <<"\n";
			}
			dp::Block *ab = NULL;
			for (int i = 0; i < eachlink[condition_alloc_path].second.size(); ++i) {
				if(blabel[eachlink[condition_alloc_path].second[i]].first == 1 || 
					blabel[eachlink[condition_alloc_path].second[i]].first == 2 ||
					blabel[eachlink[condition_alloc_path].second[i]].first == 3 ||
					blabel[eachlink[condition_alloc_path].second[i]].first == 4 ||
					blabel[eachlink[condition_alloc_path].second[i]].first == 6 ||
					blabel[eachlink[condition_alloc_path].second[i]].first == 7 ||
					blabel[eachlink[condition_alloc_path].second[i]].first == 8 ||
					blabel[eachlink[condition_alloc_path].second[i]].first == 9 ||
					blabel[eachlink[condition_alloc_path].second[i]].first == 0xa ||
					blabel[eachlink[condition_alloc_path].second[i]].first == 11 ||
					blabel[eachlink[condition_alloc_path].second[i]].first == 12 ||
					blabel[eachlink[condition_alloc_path].second[i]].first == 13 )
				{
					ab = eachlink[condition_alloc_path].second[i];
					break;
				}
			}
			...
			for(dp::Block* b : f->blocks()) {
				...
				if(seenbb.count(b->start()) > 0)
					continue;
				seenbb.insert(b->start());
			...
			Dyninst::InstructionAPI::Instruction cins; 
			Dyninst::InstructionAPI::InstructionDecoder cdecoder(f->isrc()->getPtrToInstruction(f->addr()),
				Dyninst::InstructionAPI::InstructionDecoder::maxInstructionLength,
				f->region()->getArch());
			for(dp::Block* cbb : f->blocks()) {
				Dyninst::Address cfirstAddr = cbb->start();
				Dyninst::Address clastAddr = cbb->end();
				while(cfirstAddr < clastAddr){
					cins = cdecoder.decode((unsigned char *)f->isrc()->getPtrToInstruction(cfirstAddr));
#if 1
					cout <<"\n" <<hex << cfirstAddr <<": \"" << cins.getOperation().getID() <<" "<< cins.format() << "\""<<"\n";
#endif           
					stackanalysis(f, cbb, cins, cdecoder, isleaf, cfirstAddr);
					cfirstAddr += cins.size();
				}
			}
			if(pushpoint.empty()){
				printf("no need to update rsp, just store the data behind the rsp\n");
			}
			for (const auto& p : pushpoint) {
				std::cout << "update rsp at: 0x" << std::hex << p.first.first << " -> recover rsp: 0x" << std::hex << p.first.second << std::endl;
			}
		}
		/*gettimeofday(&fetime, NULL);
        long fseconds = fetime.tv_sec - fstime.tv_sec;
